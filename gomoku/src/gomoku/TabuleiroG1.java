 ...
 * @version 2.0
 */
public final class TabuleiroG1 implements Tabuleiro {
     ...
     */
    public TabuleiroG1(byte aabTab[][]) {
        tab = new byte[DIM][DIM];
    ...
    public List<Jogada> obtemJogadasPossiveis(byte jogador) {
        ...
        Jogada aux = null;
        //se nao realizar esta jogada o jogo acaba
        //logo -e a unica possivel
        aux = obtemJogadaBoa(jogador);
        if(aux != null){
            lista = new ArrayList<Jogada>(2);
            lista.add(aux);
            return lista;
        }
        aux = obtemJogadaBoa(oponente(jogador));
        if(aux != null){
            lista = new ArrayList<Jogada>(2);
            lista.add(aux);
            return lista;
        ...
        //Mapa dividido em setores 4 para comecar
        //setor 1 - linha 0 ate 6 coluna 0 ate 6
        //setor 2 - linha 0 ate 6 coluna 6 ate 13
        //setor 3 - linha 7 ate 13 coluna 0 ate 6
        //setor 4 - linha 7 ate 13 coluna 7 ate 13
        //setores com pecas
        int particao = (DIM/2);
        boolean setor1 = true, setor2 = false, setor3 = false, setor4 = false;
        for (int linha = 0; linha < DIM; linha++) {
            for (int coluna = 0; coluna < DIM; coluna++) {
                ...
                if (verifica(jogador, aux) == 0) {
                    if(linha <= particao)
                        if(coluna <= particao) setor1 = true;
                        else setor2 = true;
                    if(linha >= particao)
                        if(coluna >= particao) setor3 = true;
                        else setor4 = true;
                }//^if (verifica(jogador, aux) == 0) 
            }//^for (int coluna = 0; coluna < DIM; coluna++) 
        }//^for (int linha = 0; linha < DIM; linha++)
        for (int linha = 0; linha < particao; linha++) {
            if(setor1){
                for (int coluna = 0; coluna < particao; coluna++) {
                    aux = new Jogada(-1, -1, linha, coluna);
                    if (verifica(jogador, aux) > 0) {
                        if (lista == null) {
            ...
            if(setor3){
                for (int coluna = particao; coluna < DIM; coluna++) {
                    aux = new Jogada(-1, -1, linha, coluna);
                    if (verifica(jogador, aux) > 0) {
                        if (lista == null) {
                            lista = new ArrayList<Jogada>(20);
        ...
        for (int linha = particao; linha < DIM; linha++) {
            if(setor2){
                for (int coluna = 0; coluna < particao; coluna++) {
                    aux = new Jogada(-1, -1, linha, coluna);
                    if (verifica(jogador, aux) > 0) {
